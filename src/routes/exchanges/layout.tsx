...
/**
 * Protected layout for exchanges routes
 * 
 ...
 * 
 * All routes under /exchanges will require authentication.
 */
...
export const useRequireAuth = routeLoader$(async (requestEvent) => {
  ...
  if (!db) {
    console.warn("[Exchanges] Database not available - redirecting to login");
  ...
  try {
    ...
    console.error("[Exchanges] Auth error:", error);
    throw requestEvent.redirect(302, "/auth/login");
  }
});
